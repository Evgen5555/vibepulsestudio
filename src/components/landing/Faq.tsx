...
const faqData: FaqItem[] = [
  {
    ...
    answer:
      "Я не обещаю золотые горы — я обещаю системный подход. Перед стартом мы вместе считаем юнит-экономику и закладываем точку безубыточности. Если вы выполняете договорённости по трафику и обратной связи, а проект не выходит на окупаемость за оговорённый срок — я бесплатно докручиваю воронку до результата. Это прописано в соглашении.",
  },
  {
    question: "Кому НЕ подойдут мои услуги?",
    answer:
      "Честно: если у вас нет понимания своей аудитории и продукта, бюджета на трафик хотя бы 30–50 тыс ₽/мес или готовности обрабатывать заявки — даже идеальный сайт не спасёт. Не работаю с серыми нишами (казино, крипто-скам, инфоцыганство). Если сомневаетесь — напишите, честно скажу, есть ли смысл.",
  },
  {
    question: "Я не разбираюсь в маркетинге — справимся?",
    answer:
      "Да, и это нормально. Моя задача — взять стратегию, аналитику и техническую часть на себя, а от вас нужны только знания о продукте и клиентах. На каждом этапе объясняю простым языком, без терминов ради терминов. После запуска даю инструкцию, как читать метрики и что с ними делать.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "В рамках утверждённого ТЗ/концепта мелкие правки не ограничены — доводим продукт до идеала вместе. Если же в процессе вы решите кардинально изменить бизнес-логику (например, вместо простого сайта сделать полноценную онлайн-школу с личными кабинетами), мы просто посчитаем это как дополнительную задачу.",
  },
  {
    question: "Какие технические гарантии?",
    answer:
      "После запуска даю 30 дней полной бесплатной техподдержки (на пакете «Под ключ» — 60). Если вылезет любой баг, слетит интеграция или что-то пойдёт не так по моей вине — исправляю моментально и в приоритете. Всё фиксируется в простом соглашении перед стартом.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Вы и только вы. После финального расчёта я передаю чистый репозиторий на GitHub и полные доступы ко всем подключённым сервисам (база данных, Telegram-боты, хостинг). Проект полностью независим — сможете развивать его дальше с любым другим разработчиком или оставить на моём сопровождении.",
  },
];
